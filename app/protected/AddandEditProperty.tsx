...
//   /* -------- Navigation helpers: always go to Properties tab -------- */
//   const goToPropertiesTab = () => {
...
//   // Hardware back handling (Android)
//   useEffect(() => {
//     const onBack = () => {
//       attemptLeave();
...
//     router.back(); // from Add button, this returns to Properties tab
//   };

//   // Bottom gutter to avoid Android nav-key overlap
//   const bottomGutter = insets.bottom + (Platform.OS === "android" ? 72 : 36);

//   // Styles (extra top spacing for sections + field blocks)
//   const s = useMemo(
//     () => ({
...
//         marginBottom: 8,
//         letterSpacing: 0.2,
//         marginTop: spacing.lg, // ⬅️ Consistent top spacing for sections
//       },
//       // Field block spacing (applies to every labeled input)
//       fieldBlock: {
//         marginTop: spacing.md * 0.75, // ⬅️ top space for each field
//         marginBottom: 10,
//       },
...
type FormValues = {
  ...
  noticeDays: string;
  facilities: string[];
  photos: string[]; // local or remote URLs
};
...
function resolveServerImageUrl(p?: string) {
  if (!p) return "";
  if (/^https?:\/\//i.test(p)) return p;
  if (IMAGE_BASES.length) return `${IMAGE_BASES[0]}/${p.replace(/^\/+/, "")}`;
  // fallback: leave as-is (works only if your API serves relative URLs from same origin)
  return p;
}
...
export default function AddandEditProperty() {
  ...
  useEffect(() => {
    ...
    // Facilities via JSON if provided
    let parsedFacilities: string[] = [];
    if (typeof params.facilities === "string") {
      try {
        const j = JSON.parse(params.facilities);
    ...
    // Images via JSON if provided (from list card)
  ...
  useEffect(() => {
    ...
    // images (string[] or [{filePath}] and also filter error objects)
    const apiPhotosRaw: string[] = Array.isArray(prop.images)
      ? prop.images
          .map((img: any) =>
            typeof img === "string"
              ? img
              : img?.filePath && typeof img.filePath === "string"
              ? img.filePath
              : ""
          )
          .filter(Boolean)
      ...
      ? [prop.filePath]
      : [];
    const apiPhotos = apiPhotosRaw.map(resolveServerImageUrl);
    ...
    if (!formTouched.current) {
      // safe to set once before user types
      reset(nextValues, { keepDirty: false, keepTouched: false });
      const sel = StateCityList.find((s) => s.name === nextValues.state);
      if (sel?.iso2) setStateIso2(sel.iso2);
    } else {
      // merge without stealing focus
  ...
  /* ---------- Navigation guards ---------- */
  const routerReplaceToList = () => router.replace("/protected/(tabs)/Properties");
  const hasUnsavedChanges = () =>
    isDirty || formTouched.current || (selectedPhotos?.length ?? 0) > 0;
  const attemptLeave = () => {
    if (hasUnsavedChanges()) {
      Alert.alert("Unsaved changes", "Discard your changes and go back?", [
        { text: "Cancel", style: "cancel" },
        { text: "Discard", style: "destructive", onPress: () => routerReplaceToList() },
      ]);
    } else {
      routerReplaceToList();
    }
  ...
  const buildFormDataAsync = async (values: FormValues, forUpdate = false) => {
    ...
    for (let idx = 0; idx < (values.photos || []).length; idx++) {
      ...
      if (Platform.OS === "web") {
        // web needs a File/Blob
        const resp = await fetch(uri);
        const blob = await resp.blob();
        const file = new File([blob], name, { type: mime });
        fd.append("images", file);
      } else {
        // native RN-style file object
        fd.append("images", {
          // @ts-ignore
          uri,
          name,
